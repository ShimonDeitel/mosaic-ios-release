...
struct PaywallView: View {
    ...
    private let benefits: [(String, String, String)] = [
        ("square.grid.2x2.fill", "The full archive", "Browse every past word, not just the last few days."),
        ("magnifyingglass", "Unlimited challenges", "Play the hidden-word find-it as many times as you like."),
        ("star.fill", "Favorites", "Keep the words you love in one place, synced across devices."),
        ("square.and.arrow.up.fill", "Share words", "Send a beautifully written word to anyone, anytime.")
    ]
            ...
            ScrollView {
                VStack(spacing: 22) {
                    VStack(spacing: 8) {
                        MosaicGlyph(size: 44)
                            .padding(.bottom, 2)
                        Text("Mosaic Pro").font(.largeTitle.weight(.heavy))
                    ...
                    VStack(spacing: 12) {
                        Button { Task { await buy() } } label: {
                            HStack {
                                ...
                                Text(working ? "Unlocking\u{2026}" : "Unlock Mosaic Pro \u{00B7} \(store.displayPrice)")
                                    .font(.headline)
                            }
                        ...
                        Text("Mosaic never tracks you. Your words stay on your device.")
